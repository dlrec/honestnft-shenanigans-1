...
def get_metadata_from_aggregator(
    ...
    if metadata_source == "rt":
        ...
        if response.status_code == 200:
            ...
            print(
                f"The selected metadata aggregator hasn't published metadata yet. (Error code: {response.status_code})"
            )
        else:
            print(
                f"Received a {response.status_code} error from the metadata aggregator API"
            )
